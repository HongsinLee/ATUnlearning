...
def compute_perturb_(model, image, label, vec_x, vec_y, range_x, range_y,
    ...
    for j in ry :
        for i in rx :
            ...
            new_image = image + i*vec_x + j*vec_y
            # pdb.set_trace()
            # distance = i**2 + j**2
            image_a = image + i*vec_x
            image_b = image + j*vec_y
            ...
            if len(images) == batch_size :
                ...
                loss_item = loss(outputs, labels).data.cpu().numpy()
                # output_a = model(images_a)
                # output_b = model(images_b)
                # loss_item = loss(output_a, labels).data.cpu().numpy() + loss(output_b, labels).data.cpu().numpy()
                loss_list.append(loss_item)
                
                # loss_item = loss(outputs, labels).data.cpu().numpy()
                # if distance in distances.keys():
                    # distances[distance] = loss
                    # loss_list.append(distances[distance])
                # else :
                    # distances[distance] = loss
                    # distances[distance] = loss_item
                    # loss_list.append(distances[distance])
...
def compute_perturb(model, image, label, vec_x, vec_y, range_x, range_y,
    ...
    loss_item = loss(outputs, torch.stack([label])).data.cpu().numpy()
    print (task_idx, label, loss_item)
    for j in ry :
        for i in rx :
            images.append(image + i*vec_x + j*vec_y)
            new_image = image + i*vec_x + j*vec_y
...
def plot_perturb_plt(rx, ry, zs, save_path, eps,
    ...
    # surf = ax.plot_surface(xs, ys, zs, rstride=1, cstride=1, facecolors=fcolors,linewidth=linewidth, antialiased=False, shade=False, alpha=0.7)
    surf = ax.plot_surface(xs, ys, zs, facecolors=fcolors)
    ...
    if not random:
        plt.savefig(save_path('loss_landscape'), format='png', dpi=300,bbox_inches='tight')
    else:
        plt.savefig(save_path('loss_landscape_rademacher'), format='png', dpi=300,bbox_inches='tight')
